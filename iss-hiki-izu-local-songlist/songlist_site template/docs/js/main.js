...
const WATCH_HISTORY_KEY = 'isshiki-izu-watch-history-v1';
...
function _svInitShareModal() {
  ...
  const rebuild = () => {
    ...
    const text = st.title ? `${st.title}` : '一色イズ 歌唱データベース';
    const x = $('#sv-share-x');
...
function _getPlaylists() {
  try { return JSON.parse(localStorage.getItem('isshiki-izu-playlists') || 'null') || []; }
  catch (_) { return []; }
}

function _savePlaylists(pls) {
  try { localStorage.setItem('isshiki-izu-playlists', JSON.stringify(pls)); } catch (_) {}
}
...
function updatePageTitle(mode) {
  ...
  el.innerHTML = '<img class="hero-title-icon" src="assets/site-icon.svg" alt="" width="32" height="32" fetchpriority="high" decoding="sync">一色イズ 歌唱データベース';
  document.title = '一色イズ 歌唱データベース';
...
const CH_INFO = {
  new: {
    name: '一色イズ',
    handle: '@isshiki_is',
    url: 'https://www.youtube.com/@%E4%B8%80%E8%89%B2%E3%82%A4%E3%82%BA',
    label: 'YouTube',
    desc: `唯一無二の主人公ボイス.ᐟ.ᐟ💫
...
リンク`,
    links: [
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M21.58 7.17a2.51 2.51 0 0 0-1.77-1.77C18.25 5 12 5 12 5s-6.25 0-7.81.4A2.51 2.51 0 0 0 2.42 7.17 26.9 26.9 0 0 0 2 12a26.9 26.9 0 0 0 .42 4.83 2.51 2.51 0 0 0 1.77 1.77c1.56.4 7.81.4 7.81.4s6.25 0 7.81-.4a2.51 2.51 0 0 0 1.77-1.77A26.9 26.9 0 0 0 22 12a26.9 26.9 0 0 0-.42-4.83ZM10 15.43V8.57L16 12l-6 3.43Z"/></svg>', label: 'YouTube', url: 'https://www.youtube.com/@%E4%B8%80%E8%89%B2%E3%82%A4%E3%82%BA' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/></svg>', label: 'X', url: 'https://x.com/isshiki_is' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true"><path d="M13.7 2h3.25a5.6 5.6 0 0 0 3.88 3.78v3.28a8.8 8.8 0 0 1-3.78-.86v6.85A6.95 6.95 0 1 1 10.1 8.1v3.5a3.47 3.47 0 1 0 3.6 3.45V2Z"/></svg>', label: 'TikTok', url: 'https://www.tiktok.com/@isshiki_is?_t=ZS-8uf8p94Zugp&_r=1' },
      { icon: '<svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true"><path d="M4 5h16v11H8l-4 4V5Z"/><path d="M8 9h8"/><path d="M8 13h5"/></svg>', label: 'マシュマロ', url: 'https://marshmallow-qa.com/5sh1w3i67zm00x2?t=35AhZ9&utm_medium=url_tex' },
    ],
