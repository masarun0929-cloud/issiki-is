...
function renderFooter() {
  ...
  const contact = SITE.contactUrl
    ...
    : '';
  footer.innerHTML = `Made with <span class="heart">♡</span> for ${escapeHtml(SITE.fanLabel)} &nbsp;·&nbsp; <a href="${escapeHtml(SOURCE_URL)}" target="_blank" rel="noopener">データソース</a>${contact}`;
}
...
function updatePageTitle(mode) {
  ...
  el.textContent = `${SITE.heroIcon} ${title}`;
