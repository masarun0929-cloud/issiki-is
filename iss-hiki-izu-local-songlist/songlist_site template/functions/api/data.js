...
function buildLives(tables) {
  ...
  for (const rows of liveSongsByEventId.values()) {
    rows.sort((a, b) => a.position - b.position);
  }
...
export async function onRequestGet({ env }) {
  try {
    ...
    const [channels, artists, songs, streams, streamSongs, songChannelStats, liveEvents, liveEventSongs] = await Promise.all([
      ...
      d1Select(env, 'live_events', 'performed_on DESC, id ASC').catch(() => []),
      d1Select(env, 'live_event_songs', 'live_event_id ASC, position ASC, id ASC').catch(() => []),
    ]);
    const tables = {
      artists,
      songs,
