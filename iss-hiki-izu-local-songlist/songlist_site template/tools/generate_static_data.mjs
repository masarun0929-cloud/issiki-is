...
function originalKeywords() {
  return String(process.env.ORIGINAL_GENRE_KEYWORDS || '一色イズ,イズ')
    .split(',')
...
function buildLives(tables) {
  ...
  const lives = (tables.live_events || []).map((event) => {
...
async function main() {
  ...
  console.log(`Generated docs/data from D1: ${combined.stats.repertoire} songs, ${combined.stats.streams} streams, ${liveData.stats.totalLives} lives`);
}
